...
class WebServer extends Worker
{
    ...
    public function run()
    {
        if (empty($this->serverRoot)) {
            echo 'server root not set, please use WebServer::addRoot($domain, $root_path) to set server root path' . PHP_EOL;
            exit(250);
        }
    ...
    public function onMessage(TcpConnection $connection)
    {
        ...
        $workerman_siteConfig = isset($this->serverRoot[$_SERVER['SERVER_NAME']])
            ? $this->serverRoot[$_SERVER['SERVER_NAME']] : current($this->serverRoot);
        ...
        if (is_file($workerman_file)) {
            // Security check.
            if (( !($workerman_request_realpath = realpath($workerman_file)) || !($workerman_root_dir_realpath = realpath($workerman_root_dir))) ||
            ...
            if ($workerman_file_extension === 'php') {
                self::executeScript($workerman_file, $connection, $workerman_siteConfig);
                return;
            }
    ...
    public static function executeScript($workerman_file, TcpConnection $connection, $workerman_siteConfig)
    {
        $workerman_root_dir = $workerman_siteConfig['root'];
    ...
    public static function sendFile(TcpConnection $connection, $file_path)
    {
        // Check 304.
        $info = stat($file_path);
